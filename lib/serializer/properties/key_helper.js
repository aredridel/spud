...
function setValue( data, key, value ) {
	iterateThroughParts( data, key, true, function( curValue, obj, subKey ) {
        /*
        // With logic of test cases 70, 73, and 76
        // If there is a duplicate key, don't overwrite it, just make an array!
        if ( obj[ subKey ] ) {
            if ( Array.isArray( obj[ subKey ] ) ) {
                obj[ subKey ].push( value );
            } else {
                obj[ subKey ] = [ obj[ subKey ], value ];
            }
        } else {
            // If there isn't a key already present, just do what you'd expect:
	       obj[ subKey ] = value;
        }
        */

        // With logic of test case 7
        obj[ subKey ] = value;
	});
}
